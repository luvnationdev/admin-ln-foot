...
export function useEditorImageUpload(editor: Editor | null) {
  ...
  const [editorImagePreview, setEditorImagePreview] = useState<string | null>(
    null
  )
  const imageInputRef = useRef<HTMLInputElement>(null)
  const [isUploading, setIsUploading] = useState(false) // Add this line

  const { uploadFile: uploadEditorImageFileAndGetUrl } = useUploadFile(
    'articles',
    editorImageFile
  )

  const handleEditorImageFileSelect = (
