// components/dashboard/mobile/OrderManagement.tsx
'use client'
...
import type { Order } from './OrderCard'; // Import the Order type from OrderCard

const orderStatusesList = ['Pending', 'Processing', 'Shipped', 'Delivered', 'Cancelled'];
...
export default function OrderManagement() {
  ...
  const handleUpdateStatus = (order: Order, newStatus: string) => {
    ...
    updateOrderMutation.mutate({ id: order.id, requestBody: payloadForUpdate as any }); // Cast as any if DTO is complex
  };
  ...
  if (ordersError) {
    ...
    return <div className="p-4 text-center text-red-600">Could not load orders. An error occurred.</div>;
  }

  return (
    <div className='space-y-4 p-4'>
      <h2 className='text-2xl font-semibold mb-6 text-center md:text-left'>Manage Orders</h2>
        ...
        <div className='space-y-4'>
          ...
          {ordersData?.map((order) => (
            <OrderCard
              ...
              isUpdatingStatus={updateOrderMutation.isPending && updateOrderMutation.variables?.id === order.id}
