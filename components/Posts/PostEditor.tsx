...
import { useNewsArticleControllerServicePostApiV1NewsArticles } from '@/lib/api-client/rq-generated/queries'
...
export default function PostEditor() {
  ...
  const { mutate: createNewsArticle, isPending: isCreatingArticle } =
    useNewsArticleControllerServicePostApiV1NewsArticles({
      onSuccess: (data) => {
        toast.success(
          `Article "${data.title}" created successfully! ` +
            (autoPublish ? 'It will be published automatically.' : '')
        )
